...
import { installments, validateAction } from "./validation";
...
export async function financeApi(request: Request, env: Pick<Cloudflare.Env, "DB">, owner: string) {
  if (request.method === "GET") {
    let categoryResult = await env.DB.prepare(categorySelect).bind(owner).all();
    if (!categoryResult.results.length) {
      await env.DB.batch(defaultCategories.map(([name, macro]) => env.DB.prepare("INSERT INTO categories (owner, name, macro) SELECT ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM categories WHERE owner = ? AND name = ? COLLATE NOCASE)").bind(owner, name, macro, owner, name)));
    ...
    if (!settings) {
      ...
      settings = await env.DB.prepare("INSERT INTO user_settings (owner, updated_at) VALUES (?, ?) ON CONFLICT(owner) DO UPDATE SET owner = excluded.owner RETURNING profile_name AS profileName, product_name AS productName, signature, currency, locale, date_format AS dateFormat, theme, density, hide_values AS hideValues, export_identity AS exportIdentity, export_owner AS exportOwner, export_generated_at AS exportGeneratedAt, export_totals AS exportTotals, export_filters AS exportFilters, export_freeze_header AS exportFreezeHeader, updated_at AS updatedAt").bind(owner, now).first<Record<string, unknown>>();
    }
  ...
  if (request.method === "POST") {
    const payload = validateAction(await readJson(request));
    ...
    if (action === "update_category") {
      ...
      const updated = await env.DB.prepare("UPDATE categories SET name = ?, macro = ? WHERE id = ? AND owner = ? RETURNING id, name, macro").bind(name, macro, id, owner).first();
      if (!updated) throw new ApiError("Categoria não encontrada.", 404);
    ...
    if (action === "save_report_note") {
      const existing = await env.DB.prepare("SELECT id FROM monthly_notes WHERE owner = ? AND month = ?").bind(owner, payload.month).all();
      if (existing.results.length > 1) throw new ApiError("Há observações duplicadas neste mês. Revise os registros antes de salvar.", 409);
      const now = new Date().toISOString();
      await env.DB.batch([
        env.DB.prepare("UPDATE monthly_notes SET note = ?, updated_at = ? WHERE owner = ? AND month = ?").bind(payload.note, now, owner, payload.month),
        env.DB.prepare("INSERT INTO monthly_notes (owner, month, note, updated_at) SELECT ?, ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM monthly_notes WHERE owner = ? AND month = ?)").bind(owner, payload.month, payload.note, now, owner, payload.month),
      ]);
      const result = await env.DB.prepare("SELECT id, month, note, updated_at AS updatedAt FROM monthly_notes WHERE owner = ? AND month = ?").bind(owner, payload.month).first();
      return json({ item: result });
    }
    if (action === "save_settings") {
      const current = normalizeSettings(payload);
      // Update only submitted preferences, without overwriting concurrent changes in another tab.
      const columns = { profileName: 'profile_name', productName: 'product_name', signature: 'signature', theme: 'theme', density: 'density', hideValues: 'hide_values', exportIdentity: 'export_identity', exportOwner: 'export_owner', exportGeneratedAt: 'export_generated_at', exportTotals: 'export_totals', exportFilters: 'export_filters', exportFreezeHeader: 'export_freeze_header' };
      const updates = [...Object.entries(columns).filter(([key]) => payload[key] !== undefined).map(([, column]) => `${column} = excluded.${column}`), 'updated_at = excluded.updated_at'].join(', ');
      const updatedAt = new Date().toISOString();
      const result = await env.DB.prepare("INSERT INTO user_settings (owner, profile_name, product_name, signature, currency, locale, date_format, theme, density, hide_values, export_identity, export_owner, export_generated_at, export_totals, export_filters, export_freeze_header, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(owner) DO UPDATE SET " + updates + " RETURNING profile_name AS profileName, product_name AS productName, signature, currency, locale, date_format AS dateFormat, theme, density, hide_values AS hideValues, export_identity AS exportIdentity, export_owner AS exportOwner, export_generated_at AS exportGeneratedAt, export_totals AS exportTotals, export_filters AS exportFilters, export_freeze_header AS exportFreezeHeader, updated_at AS updatedAt").bind(owner, current.profileName, current.productName, current.signature, "BRL", "pt-BR", "DD/MM/AAAA", current.theme, current.density, current.hideValues ? 1 : 0, current.exportIdentity ? 1 : 0, current.exportOwner ? 1 : 0, current.exportGeneratedAt ? 1 : 0, current.exportTotals ? 1 : 0, current.exportFilters ? 1 : 0, current.exportFreezeHeader ? 1 : 0, updatedAt).first<Record<string, unknown>>();
    ...
    if (action === "save_budget") {
      const selectedCategory = await categoryByName(env.DB, owner, String(payload.category || "").trim());
      if (!selectedCategory) return json({ error: "Selecione uma categoria válida." }, 400);
      const existing = await env.DB.prepare("SELECT id FROM budgets WHERE owner = ? AND month = ? AND category = ?").bind(owner, payload.month, selectedCategory.name).all();
      if (existing.results.length > 1) throw new ApiError("Há orçamentos duplicados para esta categoria e mês. Revise os registros antes de salvar.", 409);
      await env.DB.batch([
        env.DB.prepare("UPDATE budgets SET amount = ? WHERE owner = ? AND month = ? AND category = ?").bind(payload.amount, owner, payload.month, selectedCategory.name),
        env.DB.prepare("INSERT INTO budgets (owner, month, category, amount) SELECT ?, ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM budgets WHERE owner = ? AND month = ? AND category = ?)").bind(owner, payload.month, selectedCategory.name, payload.amount, owner, payload.month, selectedCategory.name),
      ]);
      const result = await env.DB.prepare("SELECT id, month, category, amount FROM budgets WHERE owner = ? AND month = ? AND category = ?").bind(owner, payload.month, selectedCategory.name).first();
      return json({ item: result }, 201);
    }
    if (action === "save_goal") {
    ...
    if (action === "delete_planning") {
      ...
      const result = await env.DB.prepare(`DELETE FROM ${table} WHERE id = ? AND owner = ? RETURNING id`).bind(Number(payload.id), owner).first();
      if (!result) throw new ApiError("Registro não encontrado.", 404);
      return json({ deleted: result });
    }
    if (action === "bulk_import") {
      ...
      for (const row of rows) {
        const category = await categoryByName(env.DB, owner, String(row.category));
        if (!category) throw new ApiError("Categoria inválida na importação: " + row.category);
        row.category = category.name; row.macro = category.macro;
      }
...
export async function backupApi(request: Request, env: Pick<Cloudflare.Env, "DB">, owner: string) {
  ...
  const results = await env.DB.batch(tables.map(table => env.DB.prepare(`SELECT * FROM ${table} WHERE owner = ?`).bind(owner)));
  const entries = tables.map((table, index) => [table, results[index].results]);
