  ...
  return <section className="transactions-page section-page">
    ...
    <section className="filter-panel">
      ...
      <div className="transaction-filters">
        ...
        <label><span>Recorrência</span><select value={recurrence} onChange={(event) => { setRecurrence(event.target.value); setPage(1); }}><option value="all">Todas</option><option>Não</option><option>Mensal</option><option>Anual</option><option>Personalizada</option></select></label>
      </div>
    </section>
    ...
    <article className="panel table-panel">
      ...
      {paged.length ? <>
        ...
        {pageCount > 1 && <div className="pagination"><button disabled={currentPage <= 1} onClick={() => setPage(currentPage - 1)}>Anterior</button><span>Página {currentPage} de {pageCount}</span><button disabled={currentPage >= pageCount} onClick={() => setPage(currentPage + 1)}>Próxima</button></div>}
