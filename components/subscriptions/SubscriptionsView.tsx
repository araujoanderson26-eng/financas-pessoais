...
import { formatDate, localIsoDate } from "@/lib/formatters";
...
export function SubscriptionsView({ subscriptions, transactions, categories, accounts, hidden, onSave, onArchive, onExport }: {
  ...
  const largest = active.reduce<Subscription | undefined>((largest, item) => !largest || item.value > largest.value ? item : largest, undefined);
  const futureInstallments = transactions.filter((item) => item.type === "saida" && Number(item.installmentTotal||1)>1 && item.date > localIsoDate()).sort((a,b)=>a.date.localeCompare(b.date));
  return <section className="subscriptions-page section-page">
    ...
    <article className="panel full-module-panel"><PanelHeader eyebrow="AGENDA FUTURA" title="Parcelas já comprometidas"/>{futureInstallments.length ? <div className="future-table">{futureInstallments.map((item)=><div key={item.id}><CalendarDays/><span><strong>{item.description}</strong><small>{formatDate(item.date)} · {item.account}</small></span><StatusBadge tone="attention">{item.installmentCurrent}/{item.installmentTotal}</StatusBadge><CurrencyValue value={item.value} hidden={hidden}/></div>)}</div> : <EmptyState title="Nenhuma parcela futura" description="Compras parceladas aparecerão aqui em ordem de vencimento."/>}</article>
  </section>;
}
