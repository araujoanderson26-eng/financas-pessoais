...
export function DashboardView({
  ...
  const balanceDelta = analytics.previous.count ? (analytics.previous.balance ? (analytics.totals.balance - analytics.previous.balance) / Math.abs(analytics.previous.balance) * 100 : null) : null;
  ...
  const commitments = [
    ...
    ...analytics.activeSubscriptions.slice(0, 5).map((item) => ({ id: `subscription-${item.id}`, date: `${selectedMonth}-${String(Math.min(item.billingDay, new Date(Number(selectedMonth.slice(0,4)), Number(selectedMonth.slice(5,7)), 0).getDate())).padStart(2, "0")}`, title: item.name, meta: `${item.category} · Assinatura mensal`, value: item.value, tone: "neutral" as const })),
