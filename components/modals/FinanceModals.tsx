...
function ModalFrame({ eyebrow, title, onClose, children }: { eyebrow: string; title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div ref={dialogRef} tabIndex={-1} className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event)=>event.stopPropagation()}><header><div><span>{eyebrow}</span><h2 id="modal-title">{title}</h2></div><button className="icon-button" onClick={onClose} aria-label="Fechar"><X/></button></header>{children}</div></div>;
}
...
function TransactionForm({ categories, accounts, editing, onSubmit, onClose }: { categories:Category[]; accounts:Account[]; editing:Transaction|null; onSubmit:(event:FormEvent<HTMLFormElement>)=>void; onClose:()=>void }) {
  ...
  return <ModalFrame eyebrow="MOVIMENTAÇÃO" title={editing?"Editar lançamento":"Novo lançamento"} onClose={onClose}><form className="form-grid premium-form" onSubmit={onSubmit}><section><h3><CircleDollarSign/>Dados principais</h3><div><label>Tipo<select name="type" defaultValue={editing?.type||"saida"} required><option value="saida">Saída</option><option value="entrada">Entrada</option></select></label><label>Data<input name="date" type="date" defaultValue={editing?.date||localIsoDate()} required/></label><label className="full">Descrição<input name="description" maxLength={editing ? 200 : 180} defaultValue={editing?.description||""} placeholder="Ex.: Supermercado" required/></label><label>Categoria<select name="category" value={category} onChange={(event)=>setCategory(event.target.value)} required>{categories.map((item)=><option key={`${item.macro}-${item.name}`}>{item.name}</option>)}</select></label><label>Natureza<input name="macro" value={nature} readOnly aria-readonly="true"/></label><label>{editing ? "Valor desta parcela" : "Valor total"}<input name="value" type="number" min="0.01" step="0.01" value={value||""} onChange={(event)=>setValue(Number(event.target.value))} placeholder="0,00" required/></label><label>Conta<select name="account" defaultValue={editing?.account||"Não informado"}><option>Não informado</option>{accounts.map((item)=><option key={item.id}>{item.name}</option>)}</select></label></div></section><section><h3><CalendarDays/>Recorrência e parcelamento</h3><p className="settings-note">A recorrência classifica este lançamento. Novas cobranças não são geradas automaticamente.</p><div><label>Recorrência<select name="recurrence" defaultValue={editing?.recurrence||"Não"}><option>Não</option><option>Mensal</option><option>Anual</option><option>Personalizada</option></select></label>{!editing&&<label>Número de parcelas<input name="installmentTotal" type="number" min="1" max="120" value={installments} onChange={(event)=>setInstallments(Math.max(1,Number(event.target.value)))}/></label>}</div>{!editing&&installments>1&&<div className="installment-preview"><Layers3/><span><strong>{installments} parcelas de aproximadamente {formatCurrency(value/installments)}</strong><small>Datas mensais a partir da data informada · total comprometido {formatCurrency(value)}</small></span></div>}</section><footer><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" type="submit">{editing?"Salvar alterações":"Salvar lançamento"}</button></footer></form></ModalFrame>;
}
...
export function CategoryDeleteDialog({ category, categories, replacementId, busy, onReplacementChange, onCancel, onConfirm }: {
  ...
  const dialogRef = useDialogFocus(Boolean(category), () => { if (!busy) onCancel(); });
  ...
  return <div className="modal-backdrop" role="presentation">
    <div ref={dialogRef} tabIndex={-1} className="confirm-dialog category-delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="category-delete-title">
